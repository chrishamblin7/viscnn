...
def get_args():
	...
	parser.add_argument('--device', type = str, default = 'cpu',
						help = "choose nvidia device 'cuda:0, cuda:1, etc.' (default cpu)")
	...
	parser.add_argument('--deepviz-edge', action='store_true', help='feature visualize edge output (warning, takes time and often fails to optimize)')
	parser.add_argument('--deepviz-channel', action='store_true', help='feature visualization using channel-wise optimization instead of neuron-wise')
...
params['deepviz_neuron'] = not args.deepviz_channel
params['deepviz_edge'] = args.deepviz_edge
...
def update_node_inputs(nodeid,image_name,target,rank_type,ablation_list,model=model_dis,max_num = params['max_node_inputs']):       
    ...
    fig.update_layout(height=260*len(top_node_edges_df), 
                      width=440,
    ...
    fig.update_coloraxes(colorscale='RdBu',reversescale=True,cmid=0,colorbar = dict(
                                                              thicknessmode = "fraction",thickness=.05, 
                                                              lenmode='fraction',len=.7)
                        )
...
def extract_subgraph_call(n_clicks,file_name,target,edge_threshold,node_threshold,rank_type,state,ablation_list,model_dis=model_dis):
    ...
    torch.save(sub_model,'prepped_models/%s/subgraphs/models/%s'%(prepped_model_folder,file_name))
