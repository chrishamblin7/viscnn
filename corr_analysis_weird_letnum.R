...
ranks_df <- read.csv('customsmall_nopool_ranks.csv')   
...
plot_dataset_activations = function(data,title) {
  return(
    ggplot(data, aes(filter_num,prune_score)) + 
      ...
      geom_vline(aes(xintercept=50)) + geom_vline(aes(xintercept=66)) + 
      geom_vline(aes(xintercept=114))  +
      theme(axis.text.x=element_blank()) +
      xlab('node') + ylab('prune score')
    
  )
}

plot_dataset_activations(ranks_df,'All Node Prune Scores of Small CNN')
...
ggplot(ranks_df, aes(prune_score)) + geom_histogram() + ggtitle('Distribution of all Prune Scores') + xlab('prune score')
