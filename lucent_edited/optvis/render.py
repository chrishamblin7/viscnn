...
from lucent_edited.optvis import objectives, transform, param
from lucent_edited.misc.io import show
...
def render_vis(
    ...
    try:
        for i in tqdm(range(1, max(thresholds) + 1), disable=(not progress)):
            ...
            #added
            #try:
            #    print(model.features[0].preadd_conv.weight.grad[0,0,0])
            #except:
...
def hook_model(model, image_f):
    ...
    def hook(layer):
        if layer == "input":
            ...
            assert layer in features, f"Invalid layer {layer}. Retrieve the list of layers with `lucent_edited.modelzoo.util.get_model_layers(model)`."
