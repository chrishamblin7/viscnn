...
if params['deepviz_neuron']:
	...
	model_dis.to(params['device']).eval()
	model = model_dis
...
print('generating deep vizualizations for edges')
for fromnode in params['imgnode_names']:
	for tonode in params['layer_nodes'][0][1]:
		edgename = str(fromnode)+'-'+str(tonode)
		print(edgename)
		image_name = fetch_deepviz_img(model_dis,str(edgename),params)

'''
for layer in range(len(params['layer_nodes'])-1):
	for fromnode in params['layer_nodes'][layer][1]:
		for tonode in params['layer_nodes'][layer+1][1]:
			edgename = str(fromnode)+'-'+str(tonode)
			print(edgename)
			image_name = fetch_deepviz_img(model_dis,str(edgename),params)
'''
