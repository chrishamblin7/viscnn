...
output_folder = 'mnist_resize'     #name of folder you want prep model to output to. Not a path here, just a name, it will appear as a folder under prepped_models/. 
...
input_img_path =  './image_data/mnist/input_images'   #Set this to the system path for the folder containing input images you would like to see network activation maps for.
rank_img_path = './image_data/mnist/ranking_images'       #Set this to a path with subfolders, where each subfolder contains a set of images. Subgraph ranks will be based on these subfolders. 

label_file_path = './image_data/mnist/labels.txt'      #line seperated file with names of label classes as they appear in image names
