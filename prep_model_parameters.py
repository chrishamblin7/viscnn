...
input_img_path =  './image_data/mnist_all/input_images'   #Set this to the system path for the folder containing input images you would like to see network activation maps for.
rank_img_path = './image_data/mnist_all/ranking_images'       #Set this to a path with subfolders, where each subfolder contains a set of images. Subgraph ranks will be based on these subfolders. 

output_folder = 'mnist_strictedges'     #name of folder you want prep model to output to. Not a path here, just a name, it will appear as a folder under prepped_models/. 
...
#MEMORY
dynamic_input=False    #If getting activations for the input images is causing memory problems, set this to true. Instead of
